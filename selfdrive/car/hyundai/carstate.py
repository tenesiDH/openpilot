...
class CarState():
  ...
  def update(self, cp, cp_mdps, cp_cam):
    ...
    self.main_on = (cp_cam.vl["SCC11"]["MainMode_ACC"] != 0) if not self.no_radar else \
                                            cp.vl['EMS16']['CRUISE_LAMP_M']
    self.acc_active = (cp_cam.vl["SCC12"]['ACCMode'] != 0) if not self.no_radar else \
    ...
    self.cruise_set_speed = cp_cam.vl["SCC11"]['VSetDis'] * speed_conv if not self.no_radar else \
