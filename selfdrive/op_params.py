...
def op_edit():  # use by running `python op_params.py` then `op_edit()`
  op_params = opParams()
  params = op_params.params
  print('Welcome to the opParams command line editor!')
  print('Here are your parameters:\n')
  while True:
    print('\n'.join(['{}. {}: {} ({})'.format(idx + 1, i, params[i], str(type(params[i]))[7:-2]) for idx, i in enumerate(params)]))
    ...
    try:
      ...
      print('New value: {} ({})\nOld value: {} ({})'.format(new_value, str(type(new_value))[7:-2], old_value, str(type(old_value))[7:-2]))
